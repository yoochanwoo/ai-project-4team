...
# 모델 저장
joblib.dump(best_rf_model, 'glass.pkl')
...
point_Na = st.slider('나트륨 (Na)', min_value=10.00, max_value=20.00, value=15.00, step=0.01)
point_Mg = st.slider('마그네슘 (Mg)', min_value=00.00, max_value=5.00, value=2.50, step=0.01)
point_Al = st.slider('알류미늄 (Al)', min_value=00.00, max_value=5.00, value=2.50, step=0.01)
point_Si = st.slider('규소 (Si)', min_value=65.00, max_value=80.00, value=72.50, step=0.01)
point_K = st.slider('칼륨 (K)', min_value=0.00, max_value=10.00, value=5.00, step=0.01)
point_Ca = st.slider('칼슘 (Ca)', min_value=5.00, max_value=20.00, value=12.50, step=0.01)
point_Ba = st.slider('바륨 (Ba)', min_value=00.00, max_value=5.00, value=2.50, step=0.01)
point_Fe = st.slider('철 (Fe)', min_value=00.00, max_value=1.00, value=0.50, step=0.01)

# 예측하기 버튼
if st.button('예측하기'):
    # 입력값을 모델에 전달
    model = joblib.load('glass.pkl')
    input_data = np.array([[point_RI, point_Na, point_Mg, point_Al, point_Si,
                           point_K, point_Ca, point_Ba, point_Fe]])
    prediction = model.predict(input_data)[0]

    # 결과 출력
    if prediction == 1:
        st.write('예측 결과: 건물_창문_부동.')
    elif prediction == 2:
        st.write('예측 결과: 건물_창문_비부동.')
    elif prediction == 3:
        st.write('예측 결과: 차량_창문_부동.')
    elif prediction == 4:
        st.write('예측 결과: 차량_창문_비부동.')
    elif prediction == 5:
        st.write('예측 결과: 용기.')
    elif prediction == 6:
        st.write('예측 결과: 식기.')
